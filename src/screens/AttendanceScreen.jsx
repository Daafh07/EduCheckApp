...
const AttendanceScreen = ({ onNavigateToSettings }) => {
  ...
  // Gebruik alleen database data - geen hardcoded fallback
  ...
  // Bereken gewogen percentage: Aanwezig=100%, Te laat=80%, Afwezig=0%
  const maxPoints = totalDays * 100; // Maximum als alles 100% zou zijn
  const earnedPoints = (presentCount * 100) + (lateCount * 80) + (absentCount * 0);
  const presentPercentage = totalDays > 0 ? Math.round(earnedPoints / maxPoints * 100) : 0;

  // Calculate stroke dash arrays for the circle (circumference ≈ 660)
  const circumference = 660;
  const presentDash = totalDays > 0 ? (presentCount / totalDays) * circumference : 0;
  const lateDash = totalDays > 0 ? (lateCount / totalDays) * circumference : 0;
  const absentDash = totalDays > 0 ? (absentCount / totalDays) * circumference : 0;
  ...
  // Toon lege state als er geen data is
      ...
      <View style={[styles.container, { backgroundColor: theme.background }]}>
        ...
        <View style={styles.content}>
          ...
          <View style={styles.emptyStateContainer}>
            <Text style={[styles.emptyStateText, { color: theme.text }]}>{t.noAttendanceData || 'Geen aanwezigheidsdata gevonden'}</Text>
          </View>
        </View>
      </View>
    );
